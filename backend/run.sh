...
#   backend/run.sh start [--fresh]  start on :8080 (or $PORT); --fresh uses a new empty database
#   Each port has its own pid/log file, so a test instance (e.g. PORT=8090) runs beside the normal one.
#   backend/run.sh stop
set -euo pipefail
DIR="$(cd "$(dirname "$0")" && pwd)"
PORT="${PORT:-8080}"; export PORT
SUFFIX=$([[ $PORT == 8080 ]] || echo "-$PORT")
PID="$DIR/target/backend$SUFFIX.pid"; LOG="$DIR/target/backend$SUFFIX.log"; JAR="$DIR/target/quickflow-backend.jar"
...
case "${1:-}" in
  ...
  start)
    ...
    [[ "${2:-}" == "--fresh" ]] && DB="jdbc:h2:mem:fresh$PORT;DB_CLOSE_DELAY=-1"
    DB_URL="$DB" nohup java -jar "$JAR" > "$LOG" 2>&1 & echo $! > "$PID"
    for _ in $(seq 60); do
      curl -sf "localhost:$PORT/actuator/health" >/dev/null && { echo "backend up on http://localhost:$PORT"; exit 0; }
      sleep 1
    done
