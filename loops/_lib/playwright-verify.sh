...
cat > "$CFG" <<JSON
{"mcpServers":{"playwright":{"type":"stdio","command":"npx","args":["-y","@playwright/mcp@latest","--headless","--isolated","--output-dir","$OUT_DIR"]}}}
JSON
...
python3 - "$RES" "$NAME" "$ROOT" <<'PY'
import json, re, subprocess, sys
res, name, root = sys.argv[1:4]
try:
