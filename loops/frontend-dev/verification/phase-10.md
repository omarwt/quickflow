...
3. Expected: the "Today's tasks" section lists "Call dentist". The "Completed today" section lists "Water plants". The "Overdue" section lists "Renew passport" with a "High" badge and a red "Due …" badge. The "Habits today" section lists "Morning run" with an unticked "Done today". The "Active plans" section shows "Focus block" with a "Rest time" countdown and a line "Next: Soon" (unless "Soon" has already started, in which case it is listed as active). The "Learning" section shows the status counts, then lists up to three cards with progress bars, starting with "Rust course" (an "In progress" badge and "0/1"), and a line "… completed in the last 7 days".
4. Read the rest time of "Focus block", wait 3 seconds and read it again. Expected: it went down (a live countdown).
5. Tick "Done today" for "Morning run". Expected: a "done for today" message appears (messages close after 5 seconds, so check straight away) and the Habits card changes to "1/1".
